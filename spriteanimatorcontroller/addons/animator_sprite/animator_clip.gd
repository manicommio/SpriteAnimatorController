...
@export var texture_color = Color(1,1,1,1)
...
#______________________________________________________
@export_category("CUSTOMIZE CLIP")
## use as custom clip
...
enum cardinal_direction {X8, X12, X16, X20, X24, X32, X36, X40}
			...
			if once:
				...
				if tiempo >= 1:
					...
					if play_backward:
						...
						frame_index = wrapi(frame_index + 1, 0, tiles_x)
		lastFrame = frame_index
	#	lastFrame = -((tiles_x) - frame - 1)
		
		
		match directions_number:
			#______________________________
			# X8 __________________________
			...
			# X12 __________________________
			...
			# X16 __________________________
			...
			# X20 __________________________
			...
			# X24 __________________________
		...
		match directions_number:
			...
			cardinal_direction.X16:
				...
				if !sprite_full_directions:
#					if (my_orientation == 9 or my_orientation == 10 or my_orientation == 11 or my_orientation == 12 or my_orientation == 13 or my_orientation == 14 or my_orientation == 15):
					if my_orientation >= 9:
						FLIP = true
					else:
						FLIP = false
...
func ExampleSprite2D() ->void:
	...
	sprite_2d.self_modulate = texture_color
...
func ExampleMaterial3D() ->void:
		...
		var uv_escala = size
		var off_set = offset
		material_3d.albedo_color = texture_color
