...
func Start() ->void:
	var father_position = father.global_position
	terrain_position = terrain.global_position + (terrain.global_transform.basis_xform(Vector2.RIGHT) * father_position.x) + (terrain.global_transform.basis_xform(Vector2.DOWN) * father_position.y)
	self.global_position = terrain_position
	self.show()
...
func Update(_delta:float) ->void:
	...
	if lift != null:
		...
		top_height = -adjust_top_height
		bottom_height = -adjust_bottom_height
