...
@export var current_health = 2000.0
var pain_healt := 2000.0
#________________________________
...
func _process(delta:float) ->void:
	...
	if state != "death":
		...
		elif pain_healt >= current_health + 400:
...
func Damage(_damage:float, _bullet:Node2D, _bullet_force:float, _shot_type:String, _shape:int) ->void:
	if state != "death" and state != "pain" and state != "heavy_damagen":
		if shield_area.broken_shield_L and shield_area.broken_shield_R:
			current_health -= _damage
		else:
			if state == "idle" or state == "looking_left" or state == "looking_right":
				state_machine.ChangeState("hunt")
		
		$AudioHit.play()
	# smoke _____________________________________________
	var health_percentage1 = (health * 70) / 100.0
	var health_percentage2 = (health * 50) / 100.0
	var health_percentage3 = (health * 30) / 100.0
	if current_health <= health_percentage1:
		if !proyector.lift.get_node("smoke_L").emitting:
			proyector.lift.get_node("smoke_L").emitting = true
	
	if current_health <= health_percentage2:
		if !proyector.lift.get_node("smoke_R").emitting:
			proyector.lift.get_node("smoke_R").emitting = true
	
	if current_health <= health_percentage3:
		if !proyector.lift.get_node("smoke_up").emitting:
			proyector.lift.get_node("smoke_up").emitting = true
