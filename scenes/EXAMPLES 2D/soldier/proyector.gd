...
func Start() ->void:
	var padre_pos = father.global_position
	terrain_position = terrain.global_position + (terrain.global_transform.basis_xform(Vector2.RIGHT) * padre_pos.x) + (terrain.global_transform.basis_xform(Vector2.DOWN) * padre_pos.y)
