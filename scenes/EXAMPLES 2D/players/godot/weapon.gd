...
var spawn_bullet = preload("res://scenes/EXAMPLES 2D/bullet/bullet.tscn")
...
func GenerateBullet(delta:float) ->void:
	...
	if tiempo_disparo >= fire_rate and cantidad_balas > 0 and !recargando:
		...
		var grados_dispersion = GameData.lista_armas[lista_armas[numero_arma]]["grados_dispersion"]
		var ajuste_dispersion = cantidad_disparo - 1
		var rot_ini = -(grados_dispersion * ajuste_dispersion) / 2
		for i in range(cantidad_disparo):
			var fuego = spawn_bullet.instantiate()
			player.world.add_child(fuego)
			var angulo_rotacion = rot_ini + ( grados_dispersion * i)
			fuego.Start(self.global_transform, angulo_rotacion, velocidad_disparo, damage, alcance, exepcion_colision, collision_mask, fuerza_disparo, (altura_vuelo + altura_underground ), altura_final, tipo_bala)
	if cantidad_balas <= 0:
		recargando = true
